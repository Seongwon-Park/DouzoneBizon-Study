...
2) 강한 결합도
필드 인젝션을 하면 스프링을 통해서만 의존성 주입이 가능하기 때문에 해당 Bean들이 스프링의 DI 컨테이너와의 강한 결합을 하게 되며, 이는 모순적이게도 Spring DI container를 사용하는 이유와 정 반대의 결과를 가져오게 된다.
...
1) 객체 불변성 확보
Constructor Injection는 Final 로 선언할 수 있다. 하지만 수정자 주입이나 일반 메소드 주입을 이용하면 Final로 선언할 수 없다. 이는 불필요하게 수정의 가능성을 열어두어 유지보수성을 떨어뜨린다. 그러므로 생성자 주입을 통해 변경의 가능성을 배제하고 불변성을 보장하는 것이 좋다.

2) 테스트 코드의 작성 용이
테스트 코드를 작성할 때에는 특정 프레임워크에 의존하는 것은 좋지 않다. 주로 테스트 코드는 순수 자바 코드로 작성하기 때문이다. 만약 아래와 같이 필드 주입을 사용한 경우에는 테스트 코드 수행 시 NPE가 발생할 것이다. (userRepository가 null이므로)
...
3) 순환 참조 에러 방지
생성자 주입을 사용하면 애플리케이션 구동 시점(객체의 생성 시점)에 순환 참조 에러를 예방할 수 있다. 왜냐하면 애플리케이션 구동 시점(객체의 생성 시점)에 에러가 발생하기 때문이다. Bean에 등록하기 위해 객체를 생성하는 과정에서 순환 참조가 발생하기 때문이다. 이는 생성자 주입과 다른 주입들간의 빈 생성 및 주입 순서에 차이가 있기 때문이다.
```
...
### 낮은 결합도? 높은 의존성?
- 지난 토론 시간에 준비가 부족했었다. 때문에 코드 분석 시간에 아무 말이나 막 뱉은 것 같다.
