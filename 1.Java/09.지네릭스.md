...
- 제네릭 : 타입을 모르지만, 타입을 정해지면 그 타입의 특성에 맞게 사용한다. (add(), addAll()과 같은 타입 파라미터와 결부된 기능도 사용하겠다.)
- 와일드 카드 : 무슨 타입인지 모르고, 무슨 타입인지 신경쓰지 않는다. 타입을 확정하지 않고 가능성을 열어둔다. (size(), clear()와 같은 인터페이스에 정의되어 있는 기능만 사용하겠다.)
