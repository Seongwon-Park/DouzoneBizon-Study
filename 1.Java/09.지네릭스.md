...
**결론은 오버로딩할 수 없다.** 컴파일에러가 발생한다. 왜냐면 Generic 타입은 컴파일러가 컴파일할 때만 사용되고 제거되기 때문이다. 따라서 위의 경우에는 메서드 중복 정의가 된다. 이럴 때 사용하는 것이 와일드카드 이며, "?" 기호를 사용한다. 이렇게 컴파일 단계에서 Generic 코드를 제거하는 이유는 Generic을 사용하지 않는 기존 코드와의 호환성을 유지하기 위함이다.
