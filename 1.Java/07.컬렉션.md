...
### 1.2.5. HashMap
HashMap 은 Map을 구현하였으므로 앞에서 살펴본 Map의 특징, Key(키) 와 Value(값)을 묶어서 하나의 데이터(entry)로 저장한다는 특징을 갖는다. 그리고 해싱을 사용하기 때문에 많은 양의 데이터를 검색하는 데에 있어서 뛰어난 성능을 보인다. 다음은 HashMap 실제 소스의 일부이다.
...
키는 전체 HashMap 에서 유일한 값을 가져야 하며, 값은 hashMap 에서 중복을 허용한다. 따라서 아래와 같은 결과를 출력한다.
```java
...
HashMap 에서의 Key / Value 를 얻는 두 가지 방법을 생각해볼 수 있다.
```java
